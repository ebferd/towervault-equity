...
?>
  <a class="nw-feature" href="/investor/news/<?= (int)$lead['id'] ?>">
    <div class="<?= news_photo_class($lead) ?>"><?= news_sky() ?><span class="nw-chip" style="position:absolute;left:14px;top:14px"><?= htmlspecialchars($lead['category']) ?></span></div>
    <div class="fb">
  ...
  <div class="nw-grid">
    <?php foreach ($rest as $p): ?>
      <a class="nw-acard" href="/investor/news/<?= (int)$p['id'] ?>">
        <div class="<?= news_photo_class($p) ?>" style="height:140px"><span class="nw-chip" style="position:absolute;left:9px;top:9px"><?= htmlspecialchars($p['category']) ?></span></div>
        <div class="ab">
